 ...
 * @return array
 */
function explodeArrayItemsByChar($array, $chars)
{
...
function getVariablesFromTemplate($template)
{
    ...
    $unwantedVariables = getUnwantedVarFromTemplate($template);
    $simpleVariables = getSimpleVariables($template, $unwantedVariables);
    $conditionVariables = getVariablesFromConditions($template, $conditionTags, $unwantedVariables);
    $loopVariables = getVariablesFromLoops($template, ['foreach'], $unwantedVariables);
    $resourceManagerVariables = getVariablesFromResourceManagerTags($template);
    ...
    $variablesWithoutFunctions = truncateUnnecessaryDataFromStrings($variables, ["empty", "isset", "in_array", "set", "trim"], FALSE);
    $variables = removeCharsFromStringArray(
        truncateUnnecessaryDataFromStrings($variablesWithoutFunctions, ["->", "=>", "::", "=\"", "'#'", ":"])
    );
    $variables = explodeArrayItemsByChar($variables, ["?", "+"]);
    $forbidenVariables = getForbiddenVariables($template);
    $variables = removeArrayBracesFromVariables(array_unique($variables, SORT_STRING));
    $variables = removeForbiddenVariablesFromArray($variables, $forbidenVariables);
 ...
 * @return array
 */
function removeForbiddenVariablesFromArray($variables, $forbidenVariables)
{
 ...
 * @return array
 */
function getSimpleVariables($template, $unwantedVariables)
{
    $variables = [];
    $pattern = '{$';
        ...
        if (strpos($line, $pattern) !== false) {
            ...
            foreach ($matches[1] as &$singleMatch) {
                ...
                if ($keySearched !== FALSE) {
                    $unwantedVariable = checkPositionVariableFromTemplate($lineNum, $keySearched);
 ...
 * @return array
 */
function getVariablesFromResourceManagerTags($template)
{
    $variables = [];
 ...
 * @return array
 */
function getVariablesFromConditions($template, $tags, $unwantedVariables)
{
    $variables = [];
    foreach ($tags as $tag) {
        $pattern = '{' . $tag;
        foreach ($template as $lineNum => $line) {
            if (strpos($line, $pattern) !== FALSE && strpos($line, "ReligisCore") == FALSE) {
                ...
                // 3. Explode &&, || a odstranění druhé části
                $splitLine = multiexplode(["&&", "||"], $line);
                $splitLine = truncateUnnecessaryDataFromStrings($splitLine, ["===", "==", "!=", "->", ">", "<", "{", "count"]);
 ...
 * @return array
 */
function getVariablesFromLoops($template, $tags, $unwantedVariables)
{
    $variables = [];
    foreach ($tags as $tag) {
        $pattern = '{' . $tag;
        ...
        foreach ($template as $lineNum => $line) {
            if (strpos($line, $pattern) !== false) {
                ...
                $splitLine = truncateUnnecessaryDataFromStrings(array($line), [" as "]);
                foreach ($splitLine as $sLine) {
                    $variable = trim($sLine);
                    $keySearched = array_search($variable, $unwantedVariables, TRUE);
 ...
 * @return array
 */
function getUnwantedVarFromTemplate($template = [])
{
    $unwantedVariables = [];
    $pattern = "{var";
...
function checkPositionVariableFromTemplate($variableLineNumber, $unwantedVariableLineNumber)
{
 ...
 */
function removeCharsFromString($string, $charsToRemove)
{
 ...
 * @return array
 */
function truncateUnnecessaryDataFromStrings($strings, $needles, $FromLeft = TRUE)
{
    $pattern = "";
    if (count($needles) == 1) {
    ...
    foreach ($strings as $string) {
        if (preg_match($pattern, $string) === 1) {
            $split = multiexplode($needles, $string);
            if ($FromLeft === TRUE) {
                $variable = $split[0];
            } else {
                $variable = end($split);
 ...
 * @return mixed
 */
function removeCharsFromStringArray($variables = [])
{
    foreach ($variables as $key => $variable) {
        unset($variables[$key]);
        $variables[] = trim(removeCharsFromString($variable, ['(', ')', '!']));
    }
    return $variables;
}


function getForbiddenVariables($template)
{
    $forbidenVariables = [];
    $pattern = '{foreach';
    foreach ($template as $lineNum => $line) {
        if (strpos($line, $pattern) !== false) {
            ...
            $variable = removeCharsFromString($line, ['}']);
